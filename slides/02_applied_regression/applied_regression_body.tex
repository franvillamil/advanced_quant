...
% ----------------------------------------------------
\imageframe{../img/cef_ols2.png}
% ----------------------------------------------------

% ----------------------------------------------------
\imageframe{../img/cef_ols3.png}
% ----------------------------------------------------
  
% ----------------------------------------------------
\imageframe{../img/cef_ols4.png}
% ----------------------------------------------------
  ...
  \begin{tikzpicture}[remember picture, overlay]
    \node[at=(current page.center)] {%
      \begin{tikzpicture}[x=10cm, y=6cm]
        % Axes
        \draw[->, thick, jet] (-0.05,0) -- (1.05,0) node[below] {$x$};
        \draw[->, thick, jet] (0,-0.05) -- (0,1.05) node[left] {$y$};

        % Scatter dots
        \foreach \x/\y in {
          0.05/0.92, 0.12/0.75, 0.18/0.60, 0.25/0.40,
          0.32/0.28, 0.40/0.18, 0.50/0.05,
          0.60/0.16, 0.68/0.30, 0.75/0.45,
          0.82/0.62, 0.90/0.88
        } {
          \fill[accent!40] (\x,\y) circle (2pt);
        }
      \end{tikzpicture}
    };
  \end{tikzpicture}
...
%           0.82/0.62, 0.90/0.88
%         } {
%           \fill[accent!20] (\x,\y) circle (2pt);
%         }

%         % Binned Averages
%         \draw[accent2, very thick] (0.00,0.90) -- (0.083,0.90);
%         \draw[accent2, very thick] (0.083,0.75) -- (0.166,0.75);
%         \draw[accent2, very thick] (0.166,0.60) -- (0.25,0.60);
%         \draw[accent2, very thick] (0.25,0.40) -- (0.333,0.40);
%         \draw[accent2, very thick] (0.333,0.28) -- (0.416,0.28);
%         \draw[accent2, very thick] (0.416,0.18) -- (0.5,0.18);
%         \draw[accent2, very thick] (0.5,0.05) -- (0.583,0.05);
%         \draw[accent2, very thick] (0.583,0.18) -- (0.666,0.18);
%         \draw[accent2, very thick] (0.666,0.30) -- (0.75,0.30);
%         \draw[accent2, very thick] (0.75,0.45) -- (0.833,0.45);
%         \draw[accent2, very thick] (0.833,0.62) -- (0.916,0.62);
%         \draw[accent2, very thick] (0.916,0.88) -- (1.0,0.88);
  ...
  \begin{tikzpicture}[remember picture, overlay]
    \node[at=(current page.center)] {%
      \begin{tikzpicture}[x=10cm, y=6cm]
        ...
        % CEF (Curve): y = (2x-1)^2
        % Note: We use the 'plot' syntax for smooth curves
        \draw[accent2, very thick, smooth, samples=100, domain=0:1] 
           plot (\x, {(2*\x-1)^2});
        \node[accent2, right] at (1,0.8) {\small CEF};
  ...
  \begin{tikzpicture}[remember picture, overlay]
    \node[at=(current page.center)] {%
      \begin{tikzpicture}[x=10cm, y=6cm]
        ...
        % OLS (Flat line): y = 0.5
        \draw[accent, very thick] (0,0.5) -- (1,0.5);
        \node[accent, right] at (1,0.5) {\small OLS};
...
% ----------------------------------------------------
\begin{frame}
\frametitle{When robust SEs are not enough: clustering}
\centering

\vspace{5pt}

\begin{itemize}[<+->]
\item Robust SEs fix heteroskedasticity
\item But what if observations are \textbf{not independent}?
\vspace{8pt}
\item Examples of clustered data:
  \begin{itemize}
  \item Students within schools
  \item Citizens within countries
  \item Repeated observations of the same unit over time
  \end{itemize}
\vspace{8pt}
\item Within a cluster, errors are correlated
\item Standard SEs (even robust) are \textbf{too small}
\item This means too many ``significant'' results
\end{itemize}

\end{frame}
% ----------------------------------------------------
\note{This extends the robust SE discussion. Robust SEs handle the case where the error variance differs across observations, but they still assume observations are independent. When data has a grouped structure -- students nested in schools, people in countries, repeated measures -- the errors within a group tend to be similar. If you ignore this, your SEs will be too small and you'll reject the null too often. The classic example: regressing student test scores on a school-level policy. You have 1,000 students but only 30 schools -- your effective sample size is closer to 30 than 1,000.}

% ----------------------------------------------------
\begin{frame}
\frametitle{Clustered standard errors in practice}
\centering

\vspace{5pt}

\begin{itemize}[<+->]
\item \textbf{Rule of thumb}: cluster at the level of treatment assignment
  \begin{itemize}
  \item Policy varies by country? Cluster by country
  \item Treatment assigned by school? Cluster by school
  \end{itemize}
\vspace{8pt}
\item In R with \texttt{modelsummary}:
  \begin{itemize}
  \item \texttt{modelsummary(model, vcov = \textasciitilde{}country)}
  \end{itemize}
\vspace{8pt}
\item Clustered SEs $\geq$ robust SEs (always at least as large)
\item With few clusters ($< 30$--$50$), even clustered SEs can be unreliable
\end{itemize}

\end{frame}
% ----------------------------------------------------
\note{Two practical points to emphasize:
\begin{enumerate}
\item The level of clustering should match the level at which the treatment or key variable varies. If you study a national policy, the ``treatment'' is at the country level, so cluster by country. If treatment is individual, you may not need clustering at all -- robust SEs suffice.
\item Clustered SEs need a reasonable number of clusters to work well. With fewer than 30--50 clusters, they can be badly behaved. In that case, consider wild cluster bootstrap or other corrections.
\end{enumerate}
The modelsummary syntax is very simple: just pass a one-sided formula with the clustering variable. This also works with fixest::feols(), which students will use later for panel data.}
