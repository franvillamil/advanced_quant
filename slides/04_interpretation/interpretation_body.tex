...
\begin{tikzpicture}[scale=0.75]
  ...
  \uncover<2->{\draw[accent, very thick] (0.7,2.9) -- (4.3,5.4);}
  \uncover<2->{\node[accent, font=\footnotesize, above] at (3.8,5.5) {$Z = 0$};}
  % Group B fitted line (positive slope)
  \uncover<2->{\draw[accent2, very thick] (5.2,1.2) -- (8.8,3.9);}
...
\item<2-> Ignoring $Z$ gives the \textbf{wrong answer} --- we need to account for it
\end{itemize}
...
\begin{itemize}[<+->]
\item \textbf{Just use plots} of predicted values
\end{itemize}
...
\item \textbf{Cook's distance}: measures how much the model changes if you remove observation $i$
  ...
  \end{itemize}
\item \textbf{Alternatively:} just loop through models dropping one observation (or group of) each time (\textit{leave-one-out})
\vspace{4pt}
