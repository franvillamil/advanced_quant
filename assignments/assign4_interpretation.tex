...
\begin{enumerate}[label=\alph*)]
  ...
  \item Interpret the coefficient on \code{log(undp\_gdp)}. Recall from the lecture that this is a level-log model: a 1\% increase in GDP is associated with a change of $\beta_1 / 100$ in the corruption index. In a comment, compute the predicted change in corruption for a \textbf{doubling} of GDP per capita (hint: $\log(2) \approx 0.693$, so the change is approximately $\beta_1 \times 0.693$).
