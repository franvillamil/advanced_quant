...
The Spatial Lag Model (also called the Spatial Autoregressive model, or SAR) posits that the outcome itself diffuses across space: $\mathbf{y} = \rho \mathbf{W}\mathbf{y} + \mathbf{X}\boldsymbol{\beta} + \boldsymbol{\varepsilon}$. Fit it using \code{lagsarlm()}:
...
\begin{enumerate}[label=\alph*)]
  ...
  \item Compute the equilibrium direct and indirect effects using \code{impacts()}. The \code{R = 500} argument draws 500 simulation samples to compute standard errors:
  \begin{lstlisting}
set.seed(42)
impacts_slm = impacts(slm_fit, listw = listw, R = 500)
summary(impacts_slm, zstats = TRUE)
  \end{lstlisting}
  In a comment, report the direct effect, the indirect effect, and the total effect of \code{log\_gdp}. How does the direct effect compare to the raw \code{log\_gdp} coefficient from the SLM output and to the OLS coefficient?
...
\begin{enumerate}[label=\alph*)]
  ...
  \item Compare OLS, SEM, and SLM using AIC. Lower AIC indicates better fit, penalized for model complexity:
  \begin{lstlisting}
AIC(ols_fit)
AIC(sem_fit)
AIC(slm_fit)
  \end{lstlisting}
  In a comment, report the three AIC values. Which model has the lowest AIC? Does this agree with your LM-test-based model choice in question 1.3b?
...
The Spatial Durbin Model (SDM) nests both SEM and SLM by including a spatially lagged dependent variable \textit{and} spatially lagged covariates. It is estimated with \code{lagsarlm()} using \code{Durbin = TRUE}:
